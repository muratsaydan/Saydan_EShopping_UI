...
Partial Class SiparisIade
	...
	Protected Sub IptalTablosu()

		Dim IptalTarihi As Date = DateAdd(DateInterval.Day, -14, Today)

		Dim cmdstr As String = "SELECT        SIPARISLER.SiparisKodu, SIPARISLER.MusteriKodu, ISNULL(SIPARISLER.OdemeYapildiMi, 0) AS OdemeYapildiMi, SIPARISLER.SiparisVerilmeTarihi, SIPARISLER.KargoyaTeslimTarihi, SIPARISLER.MusteriyeTeslimTarihi, SIPARISLER.TeslimatSehri, ODEME_ISLEMLERI.KayitTarihi, ODEME_ISLEMLERI.IadeMi FROM            SIPARISLER INNER JOIN (SELECT        Kod, SiparisKodu, Token, Status, PaymentStatus, PaidPrice, ErrorMessages, BasketID, BinNumber, CardAssociation, CardFamily, PaymentID, SystemTime, KayitTarihi, IadeMi, IadeSebebi, IadeGerceklesmeTarihi FROM            ODEME_ISLEMLERI AS ODEME_ISLEMLERI_1 WHERE        (PaymentStatus = 'SUCCESS')) AS ODEME_ISLEMLERI ON SIPARISLER.SiparisKodu = ODEME_ISLEMLERI.SiparisKodu WHERE        (ODEME_ISLEMLERI.IadeMi IS NULL OR ODEME_ISLEMLERI.IadeMi = 0) and ODEME_ISLEMLERI.KayitTarihi >= convert(datetime,'" + IptalTarihi.ToString("dd.MM.yyyy") + "',103) and MusteriKodu=" + Session("UserID").ToString
		...
		For Each dr As DataRow In dt.Rows
			...
			Else
				...
				hr.Cells.Add(hc)
				hc = New HtmlTableCell : hc.InnerHtml = "<a href=""SiparisIadeDetay.aspx?kod=" + dr("SiparisKodu").ToString + """ target=""_blank"">Seçim</a>" : hr.Cells.Add(hc)
			End If
